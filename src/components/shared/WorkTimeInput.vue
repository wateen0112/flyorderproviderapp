...
onMounted(() => {
  ...
  else { emit('update:modelValue', localValue.value); }
});
...
<template>
  <div>
    <div class="workTimes">
      ...
      <VCard v-for="(workTime, i) in localValue" :key="i"
        class="workTime overflow-hidden p-2 my-3 border">
        <VBtnGroup>
          <VBtn variant="text" color="primary" icon="mdi-plus" />
          <VBtn variant="text" color="error" icon="mdi-minus" />
          <VBtn variant="text" color="error" icon="mdi-delete"
            @click="deleteWorkTime(i)" />
        </VBtnGroup>
        <div class="days my-3">
          <VBtnToggle v-model="localValue[i].daysOfWeek"
            class="px-0 overflow-visible relative left-[10px] min-w-[50px]"
            multiple color="primary" variant="outlined">
            <VBtn v-for="(day, b) in days" :key="b" :value="day"
              variant="flat">
          ...
          <div v-for="(time, j) in workTime.times" :key="j"
            ...
            <AppDateTimePicker v-model="localValue[i].times[j].startTime"
              :config="{ enableTime: true, noCalendar: true, dateFormat: 'H:i' }" />
            <span>حتى</span>
            <AppDateTimePicker v-model="localValue[i].times[j].endTime"
              :config="{ enableTime: true, noCalendar: true, dateFormat: 'H:i' }" />
          </div>
        </div>
      </VCard>
